...
Currently, LangGraph only addresses this as part of [LangSmith Platform](/langsmith/home), not in the open source.
The reason for this is that in order to handle this we need to know how the graph is deployed, and since LangSmith Platform deals with deployment the logic needs to live there.
If you do not want to use LangSmith Platform, we describe the options we have implemented in detail below.
