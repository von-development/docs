...
The term "control plane" is used broadly to refer to the control plane UI where users create and update [LangGraph Servers](/langsmith/langgraph-server) (deployments) and the control plane APIs that support the UI experience.

When a user makes an update through the control plane UI, the update is stored in the control plane state. The [LangGraph Data Plane](/langsmith/data-plane) "listener" application polls for these updates by calling the control plane APIs.
...
The control plane and [LangGraph Data Plane](/langsmith/data-plane) "listener" application coordinate to automatically create a Postgres database for each deployment. The database serves as the [persistence layer](/oss/python/langgraph/persistence#memory-store) for the deployment.
...
The control plane and [LangGraph Data Plane](/langsmith/data-plane) "listener" application coordinate to achieve asynchronous deployments.
...
* [Postgres](/langsmith/data-plane#postgres) CPU, memory usage, and disk usage.
