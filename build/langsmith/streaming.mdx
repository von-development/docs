...
<Note>
LangGraph SDK and LangGraph Server are a part of [LangGraph Platform](/langsmith/index).
</Note>
...
LangGraph Platform allows you to join an active [background run](/langsmith/background-run) and stream outputs from it. To do so, you can use [LangGraph SDK's](/langsmith/python-sdk) `client.runs.join_stream` method:
