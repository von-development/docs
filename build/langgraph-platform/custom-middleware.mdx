...
By default, custom middleware runs before authentication and authorization logic. To run custom middleware _after_ authentication and authorization, set `middleware_order` to `auth_first` in your `http` configuration. (This customization is supported starting with API server v0.4.35 and later.)
