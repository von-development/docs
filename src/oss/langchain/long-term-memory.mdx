...
store = InMemoryStore() # (1)! [!code highlight]

store.put(  # (2)! [!code highlight]
    ("users",),  # (3)!
    "user_123",  # (4)!
    {
        "name": "John Smith",
...
def get_user_info() -> str:
    ...
    store = get_runtime().store # (6)! [!code highlight]
    user_id = get_runtime(Context).context.user_id
    user_info = store.get(("users",), user_id) # (7)! [!code highlight]
    return str(user_info.value) if user_info else "Unknown user"

agent = create_agent(
    model="anthropic:claude-3-7-sonnet-latest",
    tools=[get_user_info],
    store=store, # (8)! [!code highlight]
...
def save_user_info(user_info: UserInfo, config: RunnableConfig) -> str: # (3)!
    """Save user info."""
    # Same as that provided to `create_agent`
    store = get_store() # (4)! # [!code highlight]
    user_id = config["configurable"].get("user_id")
    store.put(("users",), user_id, user_info) # (5)! # [!code highlight]
...
agent.invoke(
    ...
    config={"configurable": {"user_id": "user_123"}} # (6)! # [!code highlight]
)
