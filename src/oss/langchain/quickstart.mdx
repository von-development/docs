...
6. **Create an run the agent** create a fully functional agent
...
<Steps>
    ...
    <Step title="Create tools">
        ...
        const getUserLocation = tool(
            (_, config) => {
                const { user_id } = config.context as { user_id: string };
    ...
    <Step title="Define response format">
        :::python
        Use structured outputs to ensure predictable, consistent responses.
        ...
        @dataclass
        class AgentResponse:
            """Final response schema for the agent."""
    ...
    <Step title="Add memory">
        ...
        the agent the agent to remember previous conversations and context.
    ...
    <Step title="Create and run the agent">
        ...
        # The `thread_id` is an identifier for a conversation thread
        # with the agent.
...
```python Expandable Full example
from dataclasses import dataclass
