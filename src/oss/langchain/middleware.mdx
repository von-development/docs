...
:::python
- `before_model`: Runs before model execution. Can update state or jump to a different node (`model`, `tools`, `__end__`)
...
:::
:::js
- `beforeModel`: Runs before model execution. Can update state or jump to a different node (`model`, `tools`, `__end__`)
...
:::
:::js
For more information, please refer to the [persistence](/oss/javascript/langgraph/persistence) documentation.
:::
</Note>
...
const toolSelector = createMiddleware({
  ...
  modifyModelRequest: (request, _state, runtime) => { // [!code highlight]
    ...
    return { ...request, tools: active }; // [!code highlight]
  },
});
...
agent.invoke({
    "messages": [HumanMessage("I need to travel to Japan next month—what visas do I need?")],
})
```
:::
:::js
