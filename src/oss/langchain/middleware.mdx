...
const vcsToolSelector = createMiddleware({
  name: "VcsToolSelector",
  contextSchema: z.object({ vcsProvider: z.enum(["github", "gitlab"]) }),
  modifyModelRequest: (request, _state, runtime) => {
    const provider = runtime.context.vcsProvider;
