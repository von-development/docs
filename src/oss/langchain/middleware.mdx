...
In this example, we have two sets of tools for creating issues in GitHub and GitLab. We use middleware to dynamically
select which set of tools to enable based on the user's VCS provider, passed in the runtime context.
...
def gitlab_create_issue(project: str, title: str) -> dict:
    ...
    # Placeholder for actual implementation
    return {"url": "https://gitlab.com/{project}/-/issues/1", "title": title}
...
def select_tools(request: ModelRequest, state: AgentState) -> ModelRequest:
    """Select tools based on the provider."""
