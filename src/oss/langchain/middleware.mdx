...
- `context_schema`: The schema of the context that the middleware requires (optional)
:::
:::js
...
Pass the full tool catalog to `create_agent` and use middleware hooks (prefer `modify_model_request`) to set `request.tools` to a small, relevant subset of tool names each turn. This avoids overflowing the prompt while keeping the validation superset intact.
:::
:::js
### How it works with `createAgent`

Pass the full tool catalog to `createAgent` and use middleware hooks (prefer `modifyModelRequest`) to set `request.tools` to a small, relevant subset of tool names each turn. This avoids overflowing the prompt while keeping the validation superset intact.
:::
...
const vcsToolGate = createMiddleware({
  ...
  modifyModelRequest: (request, _state, runtime) => {
    ...
    return { ...request, tools: [provider === "gitlab" ? "gitlab_create_issue" : "github_create_issue"] };
  },
});
...
class SelectToolsMiddleware(AgentMiddleware):
    def modify_model_request(self, request: ModelRequest, state: AgentState) -> ModelRequest:
        ...
        active_tools = select_top_k_by_similarity(content, 8) if content else full_catalog[:5]
        request.tools = [tool.name for tool in active_tools]
        return request
