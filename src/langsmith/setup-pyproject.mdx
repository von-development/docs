...
---
A LangGraph application must be configured with a [LangGraph configuration file](/langsmith/cli#configuration-file) in order to be deployed to LangGraph Platform (or to be self-hosted). This how-to guide discusses the basic steps to setup a LangGraph application for deployment using `pyproject.toml` to define your package's dependencies.
...
If you prefer using `requirements.txt` for dependency management, check out [this how-to guide](/langsmith/setup-app-requirements-txt).
</Tip>
