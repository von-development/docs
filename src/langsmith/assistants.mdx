...
<Info>
Assistants are a [LangGraph Platform](/langsmith/index) concept. They are not available in the open source LangGraph library.
</Info>
...
While configuration is available in the open source LangGraph library, assistants are only present in [LangGraph Platform](/langsmith/index). This is due to the fact that assistants are tightly coupled to your deployed graph. Upon deployment, LangGraph Server will automatically create a default assistant for each graph using the graph's default configuration settings.
