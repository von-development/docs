...
Webhooks enable event-driven communication from your LangSmith Platform application to external services. For example, you may want to issue an update to a separate service once an API call to LangSmith Platform has finished running.

Many LangSmith Platform endpoints accept a `webhook` parameter. If this parameter is specified by an endpoint that can accept POST requests, LangSmith Platform will send a request at the completion of a run.

When working with LangSmith Platform, you may want to use webhooks to receive updates after an API call completes. Webhooks are useful for triggering actions in your service once a run has finished processing. To implement this, you need to expose an endpoint that can accept `POST` requests and pass this endpoint as a `webhook` parameter in your API request.
...
To use a webhook, specify the `webhook` parameter in your API request. When the run completes, LangSmith Platform sends a `POST` request to the specified webhook URL.
...
These tools help you verify that LangSmith Platform is correctly triggering and sending webhooks to your service.
