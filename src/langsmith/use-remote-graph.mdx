...
- Separation of development and deployment: Build and test a graph locally with `CompiledGraph`, deploy it to LangSmith Platform, and then [use `RemoteGraph`](#initialize-the-graph) to call it in production while working with the same API interface.
...
- Access to [LangSmith Platform](/langsmith/home), where your graphs are developed and managed.
