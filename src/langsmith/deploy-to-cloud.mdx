---
title: LangSmith Platform on Cloud
sidebarTitle: Setup guide
icon: "cloud"
iconType: "solid"
---

Before setting up, review the [Cloud overview page](/langgraph-platform/cloud).

## Prerequisites

1. LangSmith Platform applications are deployed from GitHub repositories. Configure and upload a LangSmith Platform application to a GitHub repository in order to deploy it to LangSmith Platform.
2. [Verify that the LangGraph API runs locally](/langgraph-platform/local-server). If the API does not run successfully (i.e. `langgraph dev`), deploying to LangSmith Platform will fail as well.
...
All traffic from LangSmith Platform deployments created after January 6th 2025 will come through a NAT gateway.
